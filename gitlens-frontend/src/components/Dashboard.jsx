...
import { getAuthorColor, fmtDate, fmtShortDate, fmtMonthYear } from '../utils/constants.js'
...
import AllInsights      from './AllInsights.jsx'
    ...
    <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1, paddingBottom: 60 }}>
      ...
      <div style={S.content}>
        ...
        <div style={S.panel}>
          ...
          {tab === 'dayhour' && (
            <DayHourHeatmap hourDay={hourDay} />
          )}
          {tab === 'galaxy' && (
            contributors.length > 0
              ? <Galaxy contributors={contributors} edges={collabEdges} />
              : <Empty msg="No contributor data available" />
          )}
          {tab === 'insights' && <AllInsights data={data} />}
        </div>
      </div>
    </div>
  )
...
const S = {
  ...
  backBtn: {
    ...
    color: '#475569', fontSize: 12, padding: '6px 12px', borderRadius: 8,
  ...
  player: {
    ...
    borderRadius: 20, padding: 20, marginBottom: 22, backdropFilter: 'blur(8px)',
  },
