...
import { analyzeRepo } from '../utils/api.js'
...
export default function Landing({ onAnalyze }) {
  ...
  const handle = async () => {
    // Accept both full URL and owner/repo format
    let repoUrl = url.trim()
    if (!repoUrl) { setError('Enter a valid GitHub URL or owner/repo'); return }
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.card}>
        <div style={S.badge}>GIT HISTORY TIME TRAVELLER</div>
        ...
        {/* Feature pills */}
        <div style={S.pills}>
          {[['Cinematic Timeline'],['File Hotspots'],['Day × Hour Heatmap'],['Galaxy Graph'],['AI Insights']].map(([e])=>(
            <div key={e} style={S.pill}>{e}</div>
          ))}
        </div>
        ...
        {!loading ? (
          <div style={S.inputBox}>
            ...
            <div style={S.quickRow}>
              {QUICK.map(([r]) => (
                <button key={r} onClick={() => setUrl(r)} style={S.quickBtn}>{r}</button>
              ))}
            </div>
            ...
            <button onClick={() => onAnalyze(MOCK)} style={S.demoBtn}>
              🚀 Launch Demo with Mock Data — no backend needed
            </button>
          </div>
        ) : (
          <div style={S.loader}>
            ...
            <div style={S.hint}>Backend is parsing the repository…</div>
          </div>
        )}
      </div>
    </div>
...
const S = {
  ...
  quickBtn: {
    ...
    color: '#64748b', fontFamily: "'JetBrains Mono',monospace", cursor: 'pointer',
  },
  ...
  hint: { marginTop: 8, color: '#475569', fontFamily: "'JetBrains Mono',monospace", fontSize: 11 },
}